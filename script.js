// ============================================
// MEDITECH - Complete JavaScript - Firebase Sync Added
// ============================================
...
// ✅ ===== FIREBASE PRICE SYNC (REPLACES LOCALSTORAGE) =====
// This runs ONLY on public pages (index.html, products.html) where Firebase is initialized
if (typeof db !== 'undefined' && typeof firebase !== 'undefined') {
  db.ref('meditech_products').on('value', (snapshot) => {
    const saved = snapshot.val();
    if (!saved) return; // No data yet - show default prices from HTML
    ...
    document.querySelectorAll('.product-card').forEach((card, index) => {
      if (saved[index]) {
        ...
        if (oldEl && saved[index].old) oldEl.textContent = 'Rs. ' + saved[index].old.toLocaleString();
        if (newEl && saved[index].new) newEl.textContent = 'Rs. ' + saved[index].new.toLocaleString();
        if (discEl && saved[index].discount !== undefined) discEl.textContent = '-' + saved[index].discount + '%';
      }
    });
  });
}
// ⚠️ If Firebase isn't loaded (e.g., on pages without SDK), fallback to localStorage for backward compatibility
else if(localStorage.getItem('meditech_products')) {
  ...
  document.querySelectorAll('.product-card').forEach((card, i) => {
    if(saved[i]) {
      ...
      const newEl = card.querySelector('.price-new');
      const discEl = card.querySelector('.discount-tag');
      if (oldEl) oldEl.textContent = 'Rs. ' + saved[i].old.toLocaleString();
      if (newEl) newEl.textContent = 'Rs. ' + saved[i].new.toLocaleString();
      if (discEl) discEl.textContent = '-' + saved[i].discount + '%';
    }
  });
}
});
